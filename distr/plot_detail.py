...
def plot_grouped_bar_subplots(results, filename, ylabels, ylims=None, transpose=False):
    if TEXT_ONLY:
        return
    ...
    row_labels = results.index.values
    column_labels = results.columns
    fig, axes = plt.subplots(1, len(row_labels), figsize=(8, 3), dpi=300)
    for i, row_label in enumerate(row_labels):
        row = results.loc[row_label]
        bar = axes[i].bar(np.arange(len(column_labels)), row, zorder=3)
        for j, column_label in enumerate(column_labels):
            bar[j].set_color(colors[j % len(colors)])
            bar[j].set_edgecolor('black')
            bar[j].set_label(column_label)
            bar[j].set_hatch(hatches[j % len(hatches)])
        axes[i].set_ylabel(ylabels[i])

        # axes[i].set_xticks(np.arange(len(column_labels)))
        # axes[i].set_xticklabels(column_labels, rotation=90)
        axes[i].set_xticks([])
        axes[i].set_xlabel(row_label, labelpad=20)
        axes[i].grid(axis='y', zorder=0, linestyle='--')

        if ylims is not None:
            # axes[i].set_ylim(ylims[i])
            axes[i].set_ylim([ylims[i][0], max(row) * 1.1])
        axes[i].locator_params(axis='y', tight=True, nbins=5)

        # max_y_value = row.to_numpy().max().max()
        ...
        #     ytick_gap = math.ceil(max_y_value / num_yticks / nearest_unit) * nearest_unit
        #     axes[i].set_yticks(np.arange(num_yticks) * ytick_gap)
    ...
    lgd = fig.legend(handles, labels, fontsize=12, loc="upper center", frameon=False, ncol=len(labels), bbox_to_anchor=(0.5, 1.1), bbox_transform=fig.transFigure)
    fig.tight_layout()
    plt.savefig(f"{filename}", bbox_extra_artists=(lgd,), bbox_inches='tight')
    plt.clf()
...
def plot_stacked_bar(results, filename, ylabel="Avg Overhead (hrs)", overlay=None, overlay_ylabel=None):
    ...
    for i, row_label in enumerate(row_labels):
        ...
        for j in range(len(row)):
            ...
            ax.bar(i, row[j], bottom=sum(row[:j]), label=label, edgecolor='black', color=colors[j], zorder=3, hatch=hatches[j % len(hatches)], width=0.5)
        max_y_value = max(max_y_value, sum(row))

    if overlay:
        ax2.plot(np.arange(len(row_labels)), [overlay[k] for k in row_labels], color='darkblue', linestyle='--', marker="*", zorder=4, linewidth=2)
    ...
    ax.set_xticklabels(row_labels, rotation=0)
...
def plot_timeline(results, filename, xlabel="Hours", ylabel="Migrate Overhead (hrs)", cumulative=False, step=False, aggregate=False, scaling=1.0, markevery=1):
    if TEXT_ONLY:
        return
...
if __name__ == "__main__":
    overhead_types = ["State", "Recompute", "Persistent", "Blackout", "Queuing"]
    ...
    if "site_selection_stable" in batch_dir:
        # plot_policies = ["greedy", "mip-app", "mip"]
        plot_policies = ["greedy", "greedy-fair", 'mip-app', "mip"]
        base_policies = "avg_cov + stable + greedy + "
        suffix_policies = " + 0.0"
    

    elif "site_selection" in batch_dir:
        plot_policies = ["greedy", "greedy-fair", 'mip-app', "mip"]
    ...
    if 'subgraph_identification' in batch_dir:
        plot_policies = ['stable']
        # plot_policies = ["geo", 'stable', "cov"]
        base_policies = "avg_cov + "
        suffix_policies = " + greedy + greedy + 0.0"

    if 'subgraph_selection' in batch_dir:
        plot_policies = ["greedy", 'mip']
        base_policies = "avg_cov + cov + "
        suffix_policies = " + mip + 0.0"
    ...
    total_migrated_cores = defaultdict(lambda: np.zeros(47))
    ...
    all_power_traces = defaultdict(lambda: defaultdict(lambda: np.zeros(TIMESTEPS+1)))
    ...
    date_dirs = glob.glob(f"{batch_dir}/day_*/", recursive = False)
    for date_dir in date_dirs:
        per_date_migrated_cores = defaultdict(int)
        date = int(date_dir.replace("/", " ").strip().split("_")[-1])
        # if date == 12:
        #     continue
        per_date_overhead_percentages = defaultdict(list)
        with open(f"{date_dir}/batch_info", "r") as f:
            batch_info = json.load(f)
        ...
        for i in range(num_graphs):
            ...
            for plot_policy in plot_policies:
                if plot_policy == "greedy-fair":
                    dir_plot_policy = "greedy-fair"
                else:
                    dir_plot_policy = plot_policy
                policy_dir = f"{subgraph_dir}/{base_policies}{dir_plot_policy}{suffix_policies}"
                ...
                with open(f"{policy_dir}/power_traces", "rb") as f:
                    power_traces = pickle.load(f)
                    vb_sites = power_traces.columns

                plot_timeline(power_traces, f"{policy_dir}/power_traces.pdf", ylabel="Power (MW)", aggregate=True)
                # all_power_traces[f"day_{date}_subgraph_{i}"] = np.sum(power_traces, axis=1)
                all_power_traces[date][plot_policy] += np.sum(power_traces.to_numpy(), axis=1)
                ...
                per_date_migrated_cores[plot_policy] += migrated_cores
                ...
                per_date_overhead_percentages[plot_policy].extend(per_app_overhead_percentage)
        ...
        for plot_policy in plot_policies:
            # print(total_migrated_cores[plot_policy].shape)
            # print(per_date_migrated_cores[plot_policy].shape)
            total_migrated_cores[plot_policy] += per_date_migrated_cores[plot_policy]
            #np.concatenate((total_migrated_cores[plot_policy], per_date_migrated_cores[plot_policy]))
            
        for plot_policy, values in per_date_overhead_percentages.items():
            print(date, np.average(values), 1 + np.percentile(values, 90) / 100, 1 + np.percentile(values, 95) / 100, 1 + np.percentile(values, 99) / 100)

    TEXT_ONLY = False
        
    plot_multi_timeline(all_power_traces, f"{batch_dir}/power_traces.pdf", ylabel="Power (MW)")
    ...
    # print(np.sum(total_migrated_cores['greedy']), np.sum(total_migrated_cores['mip']))
    plot_timeline(total_migrated_cores, f"{batch_dir}/migrated_cores.pdf", ylabel="Migrated cores", cumulative=True, markevery=5)
    ...
    affected_breakdown = pd.DataFrame(total_affected_apps_percentage_breakdown).loc[plot_policies]
    affected_breakdown.rename(columns={'cofii-e4s' : "Cofii", 'db' : "DB", 'dnn' : "DL", "db_large" : 'DB-Large', "dnn_large" : 'DL-Large'},  inplace=True)
    plot_stacked_bar(affected_breakdown, f"{batch_dir}/affected_breakdown.pdf", ylabel="Percentage of Apps (%)")

    total_overhead_reports = defaultdict(dict)
    all_power_traces = pd.DataFrame(all_power_traces)
    for plot_policy, values in total_overhead_percentages.items():
        ...
        total_overhead_reports[plot_policy]['CoV of Power'] = np.average([np.std(_) / np.average(_) for _ in all_power_traces.loc[plot_policy]])
    ...
    with open(f'{output_dir}/all_traces', 'wb') as f:
        pickle.dump(all_power_traces, f)
