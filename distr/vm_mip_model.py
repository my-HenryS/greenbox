...
class NEW_MIP():
    ...
    def migrate(cur_timestamp, timestamps, interval, energy, predicted_energy_copy, vms, num_rMDCs, SLO_AVAIL = 0.99, MIP_OPT_TIME = 10000, MIP_OPT_GOAL = 0.01, init_states=None, hints=None, policy="mip", subgraph_id = 0):
        ...
        tmpVar4 = model.addVars(NUM_VM, NUM_SITE, vtype=GRB.CONTINUOUS)
        tmpVar5 = model.addVars(NUM_VM, NUM_SITE, vtype=GRB.CONTINUOUS)
        ...
        model.addConstrs(migration[t, m, n1, n2] == 0 for n1 in range(NUM_SITE) for n2 in range(NUM_SITE) for m in range(NUM_VM) for t in range(TIME))
