...
for i in range(len(ALL_PATH)):
    ...
    for slo in slos:
        ...
        for start in starts:
            if slo == 100:
                start_file = f"site{site}_slo100_powermis{powermis}_lifemis{lifemis}_dist0_start{start}_lookahead5_UTIL{util}"
            else:
        ...
        if slo == 95 and PATH == DISTR_PATH:
            total_all += 3
        elif slo == 80 and PATH == BATTERY_PATH:
            total_all -= 3
        slo_all.append(ops_all)
    designs.append(slo_all)
...
l = ["100%", "95%", "90%", "80%", "70%"]
x = np.arange(0,len(l))
plt.xlabel('Delay-Insensitive VM Uptime')
...
plt.tight_layout()
plt.savefig(f"figure/low_priority_slo_6_op.png")
plt.savefig(f"figure/low_priority_slo_6_op.pdf")
plt.clf()
