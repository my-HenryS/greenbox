...
if __name__ == "__main__":
    ...
    name = f'site{N_SITES}_slo{sys.argv[2]}_powermis{sys.argv[3]}_lifemis{sys.argv[4]}_dist{sys.argv[5]}_start{sys.argv[6]}_lookahead{sys.argv[7]}_UTIL{sys.argv[8]}_battery'
    ...
    # scheduling_policies = ["mip"]
    scheduling_policies = ["greedy", "mip-app", "mip"]
    # scheduling_policies = ["greedy", "mip"]
    ...
    for site_policy in site_policies:
        ...
        if N_SITES == 6:
            ...
            # selected_sites = ['UKH3-wind', 'DK04-wind', 'NL11-wind', 'UKK2-wind', 'UKJ4-wind', 'IS00-wind', 'FRD1-wind', 'FRH0-wind', 'FRG0-wind', 'UKJ3-wind', 'DEF0-wind', 'DK03-wind', 'NO07-wind', 'DK05-wind', 'FRI3-wind', 'NL12-wind', 'PL81-wind', 'PT20-wind', 'PT30-wind', 'RO42-wind', 'UKI5-wind', 'UKI7-wind', 'UKJ1-wind', 'NL32-wind']
        elif N_SITES == 12:
        ...
        for subgraph_policy in subgraph_policies:
            ...
            for start_time in range(start, start + REPEAT * interval, interval):
                ...
                for SUBGRAPH_FACTOR in [0.0]:#[1,2,3,4,N_SITES]:
                    ...
                    sum_trace = current_trace[selected_sites].sum(axis=1)
                    avg_p = sum(sum_trace) / (end_time - start_time)
                    cov_p = np.std(sum_trace) / avg_p
                    print(f"aggregated cov:{cov_p}")
                    cov = []
                    for site in selected_sites:
                        avg_tmp = sum(current_trace[site]) / (end_time - start_time)
                        cov_tmp = np.std(current_trace[site]) / avg_tmp
                        cov.append(cov_tmp)
                    print(f"sperate cov: {sum(cov) / len(cov)}")
                    ...
                    for placement_policy in placement_policies:
                        ...
                        for scheduling_policy in scheduling_policies:
                            ...
                            scheduler.placement(policy, start_time, end_time, RUNNING_HOURS, subgraphs, default_workloads[start_time], scaled_traces=scaled_traces, use_ray=False, cloud_util=CLOUD_UTIL, batch_name=batch_name, mip_options=MIP_OPTIONS, power_mis_ratio=POWER_MIS, name=name, lahead=LOOKAHEAD)
