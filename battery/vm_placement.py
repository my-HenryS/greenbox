    ...
    def placement(self, policy, start_time, end_time, running_hours, subgraphs, workloads, scaled_traces=None, cloud_util=1.0, use_ray=False, default_vm_arrival=None, batch_name="default", mip_options=None, power_mis_ratio=0., name="", lahead=5):
        ...
        for i in range(len(subgraphs)):
            if use_ray:
                ...
                max_t, avg_t, dist_t, completed_vm, vm_profiles, global_profile, utilization, running_vms, battery = ray.get(scheduler_finish_future)
            else:
                max_t, avg_t, dist_t, completed_vm, vm_profiles, global_profile, utilization, running_vms, battery = schedulers[i].finish()
            ...
            battery_needed = battery.copy()
            for tt in range(len(battery_needed)-1):
                for nn in range(len(battery_needed[0])):
                    battery_needed[tt,nn] = battery_needed[tt,nn] - battery[tt+1,nn] 
            battery_acc.append(np.sum(np.max(np.clip(battery_needed[:-2, :], a_min=0, a_max=None), axis=0)))
        ...
        log_msg(f"Total Battery {battery_acc}, Total Total:{sum(battery_acc)}\n")
        ...
        log_msg(f"Total Low Priority R Energy {low_priority_r_energy}, Total Total:{sum(low_priority_r_energy)}")
        ...
        with open(output_file, "a") as f:
            ...
            f.write(f"Total Battery {battery_acc}, Total Total:{sum(battery_acc)}\n")
