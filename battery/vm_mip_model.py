...
class NEW_MIP():
    ...
    @staticmethod
    def migrate(cur_timestamp, timestamps, interval, energy, predicted_energy_copy, vms, num_rMDCs, current_battery=None, SLO_AVAIL = 0.99, MIP_OPT_TIME = 10000, MIP_OPT_GOAL = 0.01, init_states=None, hints=None, policy="mip", subgraph_id = 0):
        ...
        # current_battery[cur_timestamp, :] = np.array([10000, 10000, 10000])
        log_msg("current battery", current_battery[cur_timestamp, :])
        ...
        # battery = model.addVars(TIME, NUM_SITE, vtype=GRB.CONTINUOUS, lb=0)
        ...
        tmpVar6 = model.addVars(TIME, NUM_SITE, vtype=GRB.CONTINUOUS)
        tmpVar7 = model.addVars(TIME, NUM_SITE, vtype=GRB.CONTINUOUS)
        ...
        #     model.addConstrs(battery[0,n] == 0 for n in range(NUM_SITE))
        # else:
        #     model.addConstrs(battery[0,n] == current_battery[cur_timestamp, n] for n in range(NUM_SITE))
        # for t in range(0, TIME):
        #         for n in range(NUM_SITE):
        #             battery[t, n].start == 0.0
        
        # battery capacity contraint
        # model.addConstrs(gp.quicksum(battery[t,n] for n in range(NUM_SITE)) <= 2555 for t in range(TIME))

        #does not allow migration
        model.addConstrs(migration[t,m,n1,n2] == 0 for t in range(TIME) for m in range(NUM_VM) for n1 in range(NUM_SITE) for n2 in range(NUM_SITE))

        # objective
        nr_total_power = gp.quicksum(nr_power[t, n] for t in range(TIME) for n in range(NUM_SITE))
        model.addConstrs(tmpVar5[0,n] == predicted_energy[n, cur_timestamp] + current_battery[cur_timestamp, n] for n in range(NUM_SITE))
        model.addConstrs(tmpVar5[t,n] == predicted_energy[n, t+cur_timestamp] for t in range(1, TIME) for n in range(NUM_SITE))
        # model.addConstrs(tmpVar5[t,n] == predicted_energy[n, t+cur_timestamp] + battery[t, n] for t in range(TIME) for n in range(NUM_SITE))
        ...
        model.addConstrs(tmpVar4[t,n] <= tmpVar5[t,n] + nr_power[t, n] for t in range(TIME) for n in range(NUM_SITE))
        
        
        # model.addConstrs(tmpVar6[t,n] == predicted_energy[n, t+cur_timestamp] - tmpVar4[t,n] for t in range(TIME) for n in range(NUM_SITE))
        # model.addConstrs(tmpVar7[t,n] == gp.max_(tmpVar6[t,n], 0.) for t in range(TIME) for n in range(NUM_SITE))
        # model.addConstrs(gp.quicksum(battery[t,n] for t in range(TIME)) <= gp.quicksum(tmpVar7[t,n] for t in range(TIME)) + current_battery[cur_timestamp, n] for n in range(NUM_SITE))

        # model.addConstrs(tmpVar5[t,n] == battery[t, n] + predicted_energy[n, cur_timestamp+t]-tmpVar4[t,n] for t in range(TIME) for n in range(NUM_SITE))
        # model.addConstrs(battery[t+1,n] == gp.max_(tmpVar5[t,n],0.) for t in range(TIME) for n in range(NUM_SITE))
        ...
        try:
            ...
            r_power_array = np.array([[min(vm_overhead[0, n].x + migrate_in_overhead[0, n].x + migrate_out_overhead[0, n].x, predicted_energy[n, cur_timestamp] + current_battery[cur_timestamp, n]) for n in range(NUM_SITE)]], dtype=float)
            ...
            # battery_array = np.array([[battery[t,n].x for t in range(TIME)] for n in range(NUM_SITE)])
            battery_array = np.array([min(max(current_battery[cur_timestamp, n] + predicted_energy[n, cur_timestamp] - vm_overhead[0, n].x - migrate_in_overhead[0, n].x - migrate_out_overhead[0, n].x, 0), 250) for n in range(NUM_SITE)])
            # battery_array = np.array([100, 100, 100])
        except:
        ...
        return placement_array, booted_vm_array, migration_array, nr_power_array, r_power_array, battery_array
...
class GlobalProfile():
    ...
    def __str__(self):
        return f"Total Migrations: {sum(self.migrated_vms)}, Total NR Energy: {sum(self.nr_energy_used)}, Total R Energy: {sum(self.r_energy_used)}"
...
class GreenBoxSim():
    def __init__(self, policy, energy, predicted_energy, vms, num_rMDCs, opt_time, opt_goal, slo_avail, max_timestamps=-1, interval=60, subgraph_id=-1, lahead=5):
        ...
        self.battery = np.zeros((self.T+1, self.N))
        # self.battery[0, :] = 62.5
    ...
    def greedy_decision(self, mode="NR"):
        ...
        if mode == "NR":
            ...
            # 2. Randomly shutdown VMs until it's below the vm_target_avail
            ...
            # 3. calculate NR energy used at each site   
            for n in range(self.N):
                ...
                if remaining_energy + self.battery[t, n] > 0:
                    nr_energy_array[0, n] = 0
                    r_energy_array[0, n] = demand
                    self.battery[t+1, n] = self.battery[t, n] + remaining_energy
                else:
                    nr_energy_array[0, n] = - (remaining_energy + self.battery[t,n])
                    r_energy_array[0,n] = supply + self.battery[t, n]
                    self.battery[t+1, n] = 0
            # for n in range(self.N):
        ...
        log_msg(f"timestamp:{t}, battery:{self.battery}, nr_energy:{nr_energy_array}")
    ...
    def mip_decision(self, hints=None, backlog=False, has_new_apps=True):
        ...
        ret = NEW_MIP.migrate(self.timestamp, self.lookahead, self.interval, self.energy, self.predicted_energy, self.vms, self.num_rMDCs, current_battery=self.battery, SLO_AVAIL = self.slo_avail, MIP_OPT_TIME = self.opt_time, MIP_OPT_GOAL = self.opt_goal, init_states=init_states, hints=hints, policy=self.policy, subgraph_id = self.subgraph_id)
        
        a, b, c, d, e, battery_tmp = ret
        self.battery[self.timestamp+1, :] = battery_tmp
        return a,b,c,d,e
    ...
    def migrate(self, new_vms=None, cloned=False, get_some_hints=False):
        ...
        if self.policy == "greedy":
            ...
            if ret is None:
                ...
                if running_vms is None:
                    log_msg("MIP failed; Fall back to one step MIP")
                    self.lookahead = 2
                    ret = self.mip_decision(hints=hints, backlog=False, has_new_apps=(new_vms and len(new_vms) > 0))
                    vm_placement, running_vms, migration, nr_energy, r_energy = ret
                    if running_vms is None:
                        log_msg("MIP totally failed; Fall back to greedy")
                        self.lookahead = 1
                        vm_placement, running_vms, migration, nr_energy, r_energy = self.greedy_decision()
                    back_to_greedy = True
        ...
        for n in range(self.N):
            ...
            #     log_msg(f"site {n}: power_consumption {power_consumption[n]}, energy {self.energy[n, self.timestamp]}, extra nr energy needed {nr_energy[0, n]}")
            assert(power_consumption[n] <= self.energy[n, self.timestamp] + nr_energy[0, n] + self.battery[self.timestamp, n] + EPSILON)
        ...
        if not cloned:
            for m in reversed(range(self.M)):
                ...
                if self.vm_profiles[m].lifetime_mispredict():
                    ...
                    self.vms[m].pred_lifetime = max(self.vms[m].pred_lifetime * 2, 48)
    ...
    def finish(self):
        ...
        return self.timestamp, avg_t, dist_t, self.completed_vms, all_profiles, self.global_profile, self.utilization, self.running_vms, self.battery
