...
if __name__ == "__main__":
    ...
    name = f'site{N_SITES}_slo{sys.argv[2]}_powermis{sys.argv[3]}_lifemis{sys.argv[4]}_dist{sys.argv[5]}_start{sys.argv[6]}_lookahead{sys.argv[7]}_UTIL{sys.argv[8]}_centralized_global'
        ...
        for subgraph_policy in subgraph_policies:
            ...
            for start_time in range(start, start + REPEAT * interval, interval):
                ...
                for SUBGRAPH_FACTOR in [0.0]:#[1,2,3,4,N_SITES]:
                    ...
                    # super centralized
                    tmp_subgraphs = list(subgraphs[0])
                    for i in range(1, len(subgraphs)):
                        tmp = list(subgraphs[i])[0]
                        tmp_subgraphs[0] += tmp
                    subgraphs = [tuple(tmp_subgraphs)]
