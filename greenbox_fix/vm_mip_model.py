...
class NEW_MIP():
    ...
    def migrate(cur_timestamp, timestamps, interval, energy, predicted_energy_copy, vms, num_rMDCs, SLO_AVAIL = 0.99, MIP_OPT_TIME = 10000, MIP_OPT_GOAL = 0.01, init_states=None, hints=None, policy="mip", subgraph_id = 0):
        ...
        model.addConstrs(r_power[t, n] == gp.min_(tmpVar4[t, n], predicted_energy[n, t+cur_timestamp]) for t in range(TIME) for n in range(NUM_SITE))
...
class GreenBoxSim():
    ...
    def migrate(self, new_vms=None, cloned=False, get_some_hints=False):
        ...
        if self.policy == "greedy":
            ...
            if ret is None:
                ...
                if running_vms is None:
                    ...
                    if running_vms is None:
                        self.lookahead = 1
                        vm_placement, running_vms, migration, nr_energy, r_energy = placement_hint, vm_hint, migr_hint, nr_hint, r_hint
                    back_to_greedy = True
        ...
        self.global_profile.low_priority_nr_energy_used[self.timestamp] = np.sum(nr_energy[0, :]) * low_priority_vm_power / vm_power
        self.global_profile.low_priority_r_energy_used[self.timestamp] = np.sum(r_energy[0, :]) * low_priority_vm_power / vm_power
